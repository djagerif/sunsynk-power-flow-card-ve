      ...
      return html`
        ...
        <div class="container card">
          ...
          <svg viewBox="-0.5 -0.5 457 383" preserveAspectRatio="xMidYMid meet" height="${panel === 'no' ? `${height}` : '100%'}" width="100%" xmlns:xlink="http://www.w3.org/1999/xlink" version="1.1">
            ...
            <text id="aux_load1" x="411" y="${additional_aux_load === 'one' ? '53' : '20'}" class="st3 st8" display="${show_aux === 'no' || additional_aux_load === 'no'  ? 'none' : ''}" fill="${aux_colour}" >${config?.load?.aux_load1_name ? `${config.load.aux_load1_name}` : '' }</text> 
