      ...
      return html`
        ...
        <div class="container card">
          ...
          <svg viewBox="-0.5 -0.5 457 383" preserveAspectRatio="xMidYMid meet" height="${panel === 'no' ? `${height}` : '100%'}" width="100%" xmlns:xlink="http://www.w3.org/1999/xlink" version="1.1">
            ...
            <text id="aux_one" x="411" y="82" class="st3 st8" display="${show_aux === 'no' || additional_aux_load === 'one' || additional_aux_load === 'two'? 'none' : ''}" fill="${ aux_status === 'on' || aux_status === '1' || aux_status === 'connected' ? `${aux_colour}` : `${aux_off_colour}`}" >${config?.load?.aux_name ? `${config.load.aux_name}` : 'Auxiliary' }</text> 
            ...
            <a href="#" @click=${(e) => this.handlePopup(e, config.entities.aux_connected_status)}>
              <svg xmlns="http://www.w3.org/2000/svg" id="aux_default" x="371" y="5" width="83" height="83" viewBox="0 0 24 24"><path class="${aux_type === 'default' ? '' : 'st12'}" display="${show_aux === 'no' || additional_aux_load === 'one' || additional_aux_load === 'two' ? 'none' : ''}" fill="${ aux_status === 'on' || aux_status === '1' || aux_status === 'connected' ? `${aux_colour}` : `${aux_off_colour}`}" d="M5 20v-8H2l10-9l10 9h-3v8H5m7-14.31l-5 4.5V18h10v-7.81l-5-4.5M11.5 18v-4H9l3.5-7v4H15l-3.5 7Z"/></svg>
              <svg xmlns="http://www.w3.org/2000/svg" id="aux_generator" x="374" y="5" width="74" height="74" viewBox="0 0 24 24"><path class="${aux_type === 'gen' ? '' : 'st12'}" display="${show_aux === 'no'  || additional_aux_load === 'one' || additional_aux_load === 'two' ? 'none' : ''}" fill="${ aux_status === 'on' || aux_status === '1' || aux_status === 'connected' ? `${aux_colour}` : `${aux_off_colour}`}" d="M6 3a2 2 0 0 0-2 2v11h2v1c0 .55.45 1 1 1h1c.55 0 1-.45 1-1v-1h6v1c0 .55.45 1 1 1h1c.55 0 1-.45 1-1v-1h2V5a2 2 0 0 0-2-2H6m6 4V5h6v2h-6m0 2h6v2h-6V9M8 5v4h2l-3 6v-4H5l3-6m14 15v2H2v-2h20Z"/></svg>
              <svg xmlns="http://www.w3.org/2000/svg" id="aux_inverter" x="388" y="8" width="44" height="69" viewBox="0 0 74 91"  preserveAspectRatio="xMidYMid meet"> <g transform="translate(0.000000,91.000000) scale(0.100000,-0.100000)" class="${aux_type === 'inverter' ? '' : 'st12'}" display="${show_aux === 'no'  || additional_aux_load === 'one' || additional_aux_load === 'two' ? 'none' : ''}" fill="${ aux_status === 'on' || aux_status === '1' || aux_status === 'connected' ? `${aux_colour}` : `${aux_off_colour}`}" stroke="none"> <path d="M35 887 l-27 -23 0 -404 0 -404 27 -23 c26 -23 28 -23 329 -23 284 0 305 1 327 19 l24 19 0 412 0 412 -24 19 c-22 18 -43 19 -327 19 -301 0 -303 0 -329 -23z m585 -157 l0 -80 -255 0 -255 0 0 80 0 80 255 0 255 0 0 -80z m-242 -229 c44 -34 40 -46 -14 -46 -60 0 -97 -38 -93 -94 5 -64 -23 -80 -35 -20 -9 44 24 113 63 134 35 18 34 15 21 50 -11 29 -14 30 58 -24z m110 -129 c4 -51 -19 -97 -59 -117 -27 -14 -30 -20 -23 -48 l6 -31 -51 43 c-29 24 -49 46 -46 49 3 4 23 5 44 3 58 -4 95 32 97 95 3 60 1 57 17 52 6 -3 13 -23 15 -46z"/> </g> </svg>
              <svg xmlns="http://www.w3.org/2000/svg" id="aux_oven" x="375" y="5" width="70" height="70" viewBox="0 0 32 32"><path class="${aux_type === 'oven' ? '' : 'st12'}" display="${show_aux === 'no'  || additional_aux_load === 'one' || additional_aux_load === 'two' ? 'none' : ''}" fill="${ aux_status === 'on' || aux_status === '1' || aux_status === 'connected' ? `${aux_colour}` : `${aux_off_colour}`}" d="M3 7.5A4.5 4.5 0 0 1 7.5 3h17A4.5 4.5 0 0 1 29 7.5v17a4.5 4.5 0 0 1-4.5 4.5h-17A4.5 4.5 0 0 1 3 24.5v-17Zm24 0A2.5 2.5 0 0 0 24.5 5h-17A2.5 2.5 0 0 0 5 7.5V11h22V7.5Zm0 17V13H5v11.5A2.5 2.5 0 0 0 7.5 27h17a2.5 2.5 0 0 0 2.5-2.5Zm-17-15a1.5 1.5 0 1 0 0-3a1.5 1.5 0 0 0 0 3Zm6 0a1.5 1.5 0 1 0 0-3a1.5 1.5 0 0 0 0 3ZM23.5 8a1.5 1.5 0 1 1-3 0a1.5 1.5 0 0 1 3 0ZM9 23v-6h14v6H9Zm-.5-8A1.5 1.5 0 0 0 7 16.5v7A1.5 1.5 0 0 0 8.5 25h15a1.5 1.5 0 0 0 1.5-1.5v-7a1.5 1.5 0 0 0-1.5-1.5h-15Z"/></svg>
              <svg xmlns="http://www.w3.org/2000/svg" id="aux_boiler" x="375" y="5" width="70" height="70" viewBox="0 0 24 24"><path class="${aux_type === 'boiler' ? '' : 'st12'}" display="${show_aux === 'no'  || additional_aux_load === 'one' || additional_aux_load === 'two' ? 'none' : ''}" fill="${ aux_status === 'on' || aux_status === '1' || aux_status === 'connected' ? `${aux_colour}` : `${aux_off_colour}`}" d="M9.3 10.775q0 .475.163.925t.462.825q.05-.3.2-.588t.375-.487L12 10l1.475 1.475q.225.2.375.475t.2.575q.275-.375.487-.8t.213-.9q0-.475-.15-.913t-.45-.812q-.275.125-.563.2T13 9.375q-.75 0-1.375-.425t-.95-1.125q-.3.3-.55.637t-.438.713Q9.5 9.55 9.4 9.95t-.1.825ZM12 12.1l-.425.425q-.1.1-.138.2t-.037.225q0 .25.175.4t.425.15q.25 0 .425-.15t.175-.4q0-.125-.037-.225t-.138-.2L12 12.1ZM12 5v1.9q0 .425.3.713t.725.287q.275 0 .5-.162t.4-.388l.175-.25q1.025.575 1.588 1.563t.562 2.162q0 1.75-1.25 2.963T12 15q-1.75 0-2.975-1.225T7.8 10.8q0-1.925 1.225-3.425T12 5ZM6 22q-.825 0-1.413-.588T4 20V6q0-1.65 1.175-2.825T8 2h8q1.65 0 2.825 1.175T20 6v14q0 .825-.588 1.413T18 22H6Zm0-4v2h12v-2q-.75 0-1.2.5T15 19q-1.35 0-1.763-.5T12 18q-.825 0-1.238.5T9 19q-1.35 0-1.763-.5T6 18Zm3-1q.825 0 1.238-.5T12 16q1.35 0 1.8.5t1.2.5q.75 0 1.2-.5T18 16V6q0-.825-.588-1.413T16 4H8q-.825 0-1.413.588T6 6v10q1.35 0 1.763.5T9 17Z"/></svg>
              <svg xmlns="http://www.w3.org/2000/svg" id="aux_ac" x="380" y="10" width="60" height="60" viewBox="0 0 24 24"><path class="${aux_type === 'aircon' ? '' : 'st12'}" display="${show_aux === 'no'  || additional_aux_load === 'one' || additional_aux_load === 'two' ? 'none' : ''}" fill="${ aux_status === 'on' || aux_status === '1' || aux_status === 'connected' ? `${aux_colour}` : `${aux_off_colour}`}" d="M6.59.66c2.34-1.81 4.88.4 5.45 3.84c.43 0 .85.12 1.23.34c.52-.6.98-1.42.8-2.34c-.42-2.15 1.99-3.89 4.28-.92c1.81 2.34-.4 4.88-3.85 5.45c0 .43-.11.86-.34 1.24c.6.51 1.42.97 2.34.79c2.13-.42 3.88 1.98.91 4.28c-2.34 1.81-4.88-.4-5.45-3.84c-.43 0-.85-.13-1.22-.35c-.52.6-.99 1.43-.81 2.35c.42 2.14-1.99 3.89-4.28.92c-1.82-2.35.4-4.89 3.85-5.45c0-.43.13-.85.35-1.23c-.6-.51-1.42-.98-2.35-.8c-2.13.42-3.88-1.98-.91-4.28M5 16h2a2 2 0 0 1 2 2v6H7v-2H5v2H3v-6a2 2 0 0 1 2-2m0 2v2h2v-2H5m7.93-2H15l-2.93 8H10l2.93-8M18 16h3v2h-3v4h3v2h-3a2 2 0 0 1-2-2v-4a2 2 0 0 1 2-2Z"/></svg>
              <svg xmlns="http://www.w3.org/2000/svg" id="aux_pump" x="380" y="15" width="60" height="70" viewBox="0 0 24 24"><path class="${aux_type === 'pump' ? '' : 'st12'}" display="${show_aux === 'no'  || additional_aux_load === 'one' || additional_aux_load === 'two' ? 'none' : ''}" fill="${ aux_status === 'on' || aux_status === '1' || aux_status === 'connected' ? `${aux_colour}` : `${aux_off_colour}`}" d="M3 17h4.1q-.425-.425-.787-.925T5.675 15H3v2Zm9 0q2.075 0 3.538-1.463T17 12q0-2.075-1.463-3.538T12 7Q9.925 7 8.462 8.463T7 12q0 2.075 1.463 3.538T12 17Zm6.325-8H21V7h-4.1q.425.425.788.925T18.325 9ZM1 20v-8h2v1h2.075q-.05-.25-.063-.488T5 12q0-2.925 2.038-4.963T12 5h9V4h2v8h-2v-1h-2.075q.05.25.063.488T19 12q0 2.925-2.038 4.963T12 19H3v1H1Zm2-3v-2v2Zm18-8V7v2Zm-9 3Zm0 3q-.825 0-1.413-.588T10 13q0-.575.238-1.137t.912-1.613L12 9l.85 1.25q.675 1.05.913 1.613T14 13q0 .825-.588 1.413T12 15Z"/></svg>
              ...
              <g display="${show_aux === 'no'  || additional_aux_load === 'one' || additional_aux_load === 'two' ? 'none' : ''}" >
                <foreignObject x="375" y="5" width="85" height="85" style="position: fixed; ">
                  <body xmlns="http://www.w3.org/1999/xhtml" >
                    <div style="position: fixed; ">
                      <ha-icon icon="${aux_type}" class="${ aux_status === 'on' || aux_status === '1' || aux_status === 'connected' ? 'aux-icon' : 'aux-off-icon' }" ></ha-icon>
                    </div>
                  </body>
                </foreignObject>
              </g>
